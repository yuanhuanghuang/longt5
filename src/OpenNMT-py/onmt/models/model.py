...
from transformers import T5Model, T5Tokenizer, BartTokenizer, BartModel, AutoTokenizer, LongT5Model


class LTModel(LongT5Model):
    """
...
class LTEncModel(LongT5Model): #longT5 enc multichoice model
    """
    Core trainable object in OpenNMT. Implements a trainable interface
    for a simple, generic encoder + decoder model.
    Args:
    ...
    def forward(
        ...
        bottled_output = bottled_output.view(len(input_ids), 4)
...
class NMTModel(T5Model):
    ...
    def forward(
        ...
        fdecoder = self.decoder

        decoder_outputs = fdecoder(
            input_ids=decoder_input_ids,
            attention_mask=decoder_attention_mask,
            inputs_embeds=decoder_inputs_embeds,
            past_key_values=past_key_values,
