...
def get_fields(
    ...
    src_field_kwargs = {"n_feats": n_src_feats,
                        ...
                        "pad": pad, "unk":unk, "bos": None, "eos": None,
...
def _build_field_vocab(field, counter, size_multiple=1, **kwargs):
    ...
    all_specials = [field.pad_token]
    for i in range(99):
        all_specials.append("[unused{}]".format(i))
    all_specials += [field.unk_token, field.init_token, field.eos_token]
    specials = [tok for tok in all_specials if tok is not None]
    field.vocab = field.vocab_cls(counter, specials=specials, **kwargs)

    unk_index = specials.index(field.unk_token)
    field.vocab.unk_index = unk_index
    field.vocab.stoi.default_factory = lambda: unk_index
    if size_multiple > 1:
...
class DatasetLazyIter(object):
    ...
    def __init__(self, dataset_paths, fields, batch_size, batch_size_fn,
                 batch_size_multiple, device, is_train, pool_factor, cut_path, dataset_paths2,
        ...
        self.yield_raw_example = yield_raw_example
        self.pool_factor = pool_factor
        self._paths2 = dataset_paths2
        if cut_path is not None:
            import json
            self.valid_idxs = json.load(open(cut_path))
        else:
...
#        logger.info('Loading dataset from %s' % path)
        ...
        if self._paths2 is not None:
            cur_dataset2 = torch.load(self._paths2[0])
            if self.valid_idxs is not None:
                cur_dataset2.examples = [cur_dataset2[i] for i in self.valid_idxs]
            cur_dataset.examples += cur_dataset2.examples 
...
def build_dataset_iter(corpus_type, fields, opt, is_train=True, multi=False):
    ...
    if opt.data2 is not None:
        dataset_glob2 = opt.data2 + '.' + corpus_type + '.[0-9]*.pt'
        dataset_paths2 = list(sorted(
            glob.glob(dataset_glob2),
            key=lambda p: int(p.split(".")[-2])))
    else:
        dataset_paths2 = None
    ...
    return DatasetLazyIter(
        ...
        is_train,
        opt.pool_factor,
        opt.cut_path if is_train else None,
        dataset_paths2,
