...
def get_metrics(
    ...
    f1 = round(f1, 2)
    return exact_match, f1, predicted_bags, gold_bags
...
def evaluate_json(
    ...
    for _, annotation in annotations.items():
        for qa_pair in annotation["qa_pairs"]:
            ...
            if query_id in predicted_answers:
                ...
                if "<extra_id_0>" in predicted:
                    predicted = predicted.split("<extra_id_0>")
                ...
                for answer in candidate_answers:
                    ...
                    em_score, f1_score, predicted_bags, gold_bags = get_metrics(predicted, gold_answer)
                    ans_pairs.append({"predicted_bags": predicted_bags[0], "gold_bags": gold_bags[0], "gold_type": gold_type})
    ...
    return global_em, global_f1, ans_pairs
...
def evaluate_prediction_file(
    ...
    global_em, global_f1, ans_pairs = evaluate_json(annotations, predicted_answers)

    # Output predictions to file if an output path is given
    if output_path is not None:
        output_dict = {"global_em": global_em, "global_f1": global_f1, "ans_pairs": ans_pairs}

        with open(output_path, "w", encoding="utf8") as outfile:
            json.dump(output_dict, outfile, indent=2, separators=(',', ':'))

    return (global_em, global_f1)


def generate(in_file, out_file_prefix, filter_file=None):
    data = json.load(open(in_file))
    if filter_file is not None:
        filter_idxs = json.load(open(filter_file))
    ...
    for i, d in data.items():
        for qa in d['qa_pairs']:
            if filter_file is not None and qa['query_id'] not in filter_idxs:
                continue
            source.append(qa['question'] + " context: " + d['passage'])
            if len(qa['answer']['number']) > 0:
                answer = qa['answer']['number']
            elif len(qa['answer']['spans']) > 0:
                answer = ' <extra_id_0> '.join(qa['answer']['spans'])
            else:
                answer = (' '.join([qa['answer']['date']['day'], qa['answer']['date']['month'], qa['answer']['date']['year']])).strip()
            target.append(answer)

    with open(out_file_prefix + "_source.txt", 'w') as f:
        for i, d in enumerate(source):
            f.write(d)
            f.write("\n")
    with open(out_file_prefix + "_target.txt", 'w') as f:
        for i, d in enumerate(target):
            f.write(d)
            f.write("\n")

def format_test_drop(pred_file, gt_file, filter_file=None):
    pred_dict = {}
    pred = [d.strip() for d in open(pred_file).readlines()]
    ...
    for i, d in gt.items():
        for qa in d['qa_pairs']:
            ...
            if filter_file is not None and query_id not in filter_idxs:
                continue
            pred_dict[query_id] = pred[count]
            count += 1
    json.dump(pred_dict, open("dev_pred_drop_t5_s1.json", 'w', encoding="utf-8"))

def test_drop(pred_file, gt_file):
    pred = [d.strip() for d in open(pred_file).readlines()]
    gt = json.load(open(gt_file))
    target = []
    ...
    for i, d in gt.items():
        for qa in d['qa_pairs']:
            ...
            if len(qa['answer']['number']) > 0:
                ...
                answer = (' '.join([qa['answer']['date']['day'], qa['answer']['date']['month'], qa['answer']['date']['year']])).strip()
            target_tmp.append(answer)
            for v_ans in qa["validated_answers"]:
                if len(v_ans['number']) > 0:
                    answer = v_ans['number']
                elif len(v_ans['spans']) > 0:
                    answer = ', '.join(v_ans['spans'])
                else:
                    answer = (' '.join([v_ans['date']['day'], v_ans['date']['month'], v_ans['date']['year']])).strip()
                target_tmp.append(answer)
            target.append(target_tmp)

    assert len(target) == len(pred)
    count = 0
    for pd, t in zip(pred, target):
        if pd in t:
            count += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default="dev_pred_drop_t5_s1.json",
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default="drop_results.json",
        help="location of the output metrics file",
    )

    args = parser.parse_args()
    format_test_drop("../src/OpenNMT-py/preds/qdmr_single_dev_pred_drop_qdmr_single_longt5_s1.txt", "drop_dataset_dev.json", 'qdmr_idx_dev.json')
    evaluate_prediction_file(args.prediction_path, args.gold_path, args.output_path)

#    generate('drop_dataset_train.json', 'qdmr_single_train', 'qdmr_idx_train.json')
#    generate('drop_dataset_dev.json', 'qdmr_single_dev', 'qdmr_idx_dev.json')
